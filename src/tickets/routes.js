...
export default {
    ...
    get: {
      handler: async(req, res) => {
        ...
        tickets = tickets.map((ticket) => {
          if (!user || (user.email !== 'kevin@terrapinticketing.com' || String(ticket.ownerId) !== String(user._id))) {
  ...
  ['/payment/:id']: {
    post: {
      ...
      handler: async(req, res) => {
        return res.send({ error: 'Buys and Sells have been disabled for this event' })
        // let { user, Integration } = req.props
        // const { id } = req.params
        // const ticketId = id
